...
window.onload = function(){
    ...
    xhttp.addEventListener("load",function(event){
        ...
        var bookstore = JSON.parse(response);
        processBookstoreList(bookstore);
    });
    ...
    if(data.length==0){
        ...
        data.forEach(function(row,index) {
            ...
            for(column in row){
                ...
                if(column ==="img_url"){
                    text_html = text_html+"<td><img alt = 'img' width='50px' height='50px' src='"+server+'images/'+row[column]+"'></td>";
                }
                else{
